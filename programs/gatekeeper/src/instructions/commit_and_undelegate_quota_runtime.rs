...
#[commit]
#[derive(Accounts)]
pub struct CommitAndUndelegateQuotaRuntime<'info> {
...
pub fn handler(ctx: Context<CommitAndUndelegateQuotaRuntime>) -> Result<()> {
    require!(
        ctx.accounts.quota_runtime.is_delegated,
        GatekeeperError::RuntimeNotDelegated
    );

    ctx.accounts.quota_runtime.is_delegated = false;
    ctx.accounts.quota_runtime.delegated_validator = DEFAULT_VALIDATOR.into();
    ctx.accounts.quota_runtime.commit_frequency_ms = 0;
